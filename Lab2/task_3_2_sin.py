...
print("\n==== FIND BEST KERNEL FOR NOISY SIN(2x) DATA ====")

# noisy sin(2x) data generation with variance 0.1
...
for n_hidden in n_hidden_vec:
    ...
    data_sin_kernels[n_hidden]['genralisation_gap'] = gen_gap_vec
...
for n_nodes, n_data in data_sin_kernels.items():
...
y_best_prediction = best_combo['model'].predict(X_test_sin)
...
# TODO Train best for sinus(2x) for least square and delta rule

centers = []
centers_gaps = (X_train_sin_length/best_combo['nodes'])*(1+1/best_combo['nodes'])
...
rbf_net.set_centers(centers)
train_mse, valid_mse = rbf_net.fit_least_square(X_train_sin, Y_train_sin, X_test_sin, Y_test_sin)
...
plt.figure(figsize=(10, 6))
plt.scatter(X_train_sin, Y_train_sin, s=10, label=f'Training data')
plt.plot(X_test_sin, y_predict_least_square, color='red', label=f'Least square model')
plt.plot(X_test_sin, y_best_prediction, color='green', label=f'Delta rule model')
...
# TODO Train on random centers for sin
centers = []
centers_gaps = (X_train_sin_length/best_combo['nodes'])*(1+1/best_combo['nodes'])
for n in range(best_combo['nodes']):
    centers.append(min(X_train_sin) + n*centers_gaps)
...
plt.figure(figsize=(10, 6))
plt.scatter(X_train_sin, Y_train_sin, s=10, label=f'Training data')
plt.plot(X_test_sin, y_predict_random_centers, color='red', label=f'Random center prediction')
plt.plot(X_test_sin, y_predict_my_centers, color='green', label=f'Our method center prediction')
...
#TODO Generate clean data

_, _, X_test_clean, Y_test_clean = generate_data_sin_2x()

# TODO test sin on clean data
...
# TODO Train perceptron on noised data sin(2x)

mlp_nn = mlp([X_train_sin.shape[1]] + [best_combo['nodes']] + [1] )

start_time = time.time()
MLP_train_mse, MLP_valid_mse = mlp_nn.train_with_validation_set(X_train_sin.T, Y_train_sin.T, X_test_sin.T, Y_test_sin.T, 3000)
elapsed_time = time.time() - start_time
...
y_predict_mlp = mlp_nn.predict(X_test_sin.T)
if y_best_prediction.shape != y_predict_mlp.shape:
     y_predict_mlp = y_predict_mlp.T

plt.figure(figsize=(10, 6))
plt.scatter(X_train_sin, Y_train_sin, s=10, label=f'Training data')
plt.plot(X_test_sin, y_predict_mlp, color='red', label=f'MLP prediction')
