...
# Generate Data in order to have all the data points on the same quadrant
###### Parameters
n = 100  # number of points
# Class A (mean and standard deviation)
mA = np.array([ 2.0, 1.0]) #(x,y)
sigmaA = 0.2 #how data are sparsed around the mean

# Class B (mean and standard deviation)
mB = np.array([4.0, 3.0]) #(x,y) centring the class B around (0, 0) to show the effect of bias=0
...
plt.title(f"Delta Rule: Batch mode without bias and data on the same quadrant")
plt.legend()
